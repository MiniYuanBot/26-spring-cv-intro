...
def bn_backward(dout, cache):
    ...
    s1 = np.sum(dx_hat, axis=0)  # (D,)
    s2 = np.sum(dx_hat * x_hat, axis=0)  # (D,)
    dx = (N * dx_hat - s1 - x_hat * s2) / (N * np.sqrt(sample_var + eps))  # (N, D)
    # ----------------TODO------------------
...
if __name__ == "__main__":
    ...
    for i in range(9):
        train_data[i, :] = cv2.imread(os.path.join(
            HW_dir, "mnist_subset", f"{i}.png"), cv2.IMREAD_GRAYSCALE).reshape(-1)/255.
    gt_y = np.zeros((9, 1))
    gt_y[0] = 1

    val_data = np.zeros((1, 784))
    val_data[0, :] = cv2.imread(os.path.join(
        HW_dir, "mnist_subset", "9.png"), cv2.IMREAD_GRAYSCALE).reshape(-1)/255.
    val_gt = np.zeros((1, 1))

    np.random.seed(14)

    # Intialize MLP  (784 -> 16 -> 1)
    MLP_layer_1 = np.random.randn(784, 16)   # (784, 16)
    MLP_layer_2 = np.random.randn(16, 1)  # (16, 1)

    # Initialize gamma and beta
    gamma = np.random.randn(16)  # (16,)
    beta = np.random.randn(16)  # (16,)
